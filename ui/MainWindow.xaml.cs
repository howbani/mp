...
using MP.Forwarding;
using MP.ExpermentsResults.Energy_consumptions;
using MP.ExpermentsResults.Lifetime;
using MP.ControlPlane.NOS.TC;
using MP.DataPlane.NeighborsDiscovery;
using MP.ui.conts;
using MP.MergedPath.DiagonalVirtualLine;
    ...
    public partial class MainWindow : Window
    {
        ...
        private void TimeConuter()
        {
            if (PublicParamerters.SimulationTime <= Settings.Default.StopSimlationWhen + PublicParamerters.MacStartUp)
            {
                ...
                Title = "MP:" + PublicParamerters.SimulationTime.ToString();
            }
            else
            {
        ...
        {
            //AverageNumberOfHops
            AverageEnergyConsumption.ExpermentalEnergyConsumption exp = new AverageEnergyConsumption.ExpermentalEnergyConsumption();
            exp.SendRandTwoPoints(1); // five sinks
            ...
            foreach (Packet packet in PublicParamerters.FinishedRoutedPackets )
            {
                ...
                {
                    coun += 1;
                    sum += packet.UsedEnergy_Joule;
                }
            }

            double av = sum / coun;
            Console.WriteLine(coun+ " :Average UsedEnergy_Joule:" + av);
        }
    }
}
