...
namespace MP.ExpermentsResults.Energy_consumptions
{
    ...
    public partial class ExpReport : Window
    {
        ...
        public ExpReport(MainWindow _mianWind)
        {
            ...
            List.Add(new ValParPair() { Par = "# Diagonal Virtual Line Construction Packet", Val = PublicParamerters.NumberOfDiagonalVirtualLineConstructionPacket.ToString() });

            // others:
            List.Add(new ValParPair() { Par = "Max Clustering Threshold", Val = Settings.Default.MaxClusteringThreshold.ToString() });
            List.Add(new ValParPair() { Par = "Coordination Type", Val = Settings.Default.CoordinationType });
            List.Add(new ValParPair() { Par = "Is Energy Considered", Val = Settings.Default.ConsiderEnergy.ToString() });
            List.Add(new ValParPair() { Par = "Sinks start at center", Val = Settings.Default.SinksStartAtNetworkCenter.ToString() });
            List.Add(new ValParPair() { Par = "Protocol", Val = "Merged Path" });
